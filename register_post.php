...
/* Information from form. Used HTTP POST method to catch variable inputs from users. */
...
if ($validated_email) {
    if (strlen($password) >= 8 && $pass_cap == 1 && $pass_small == 1 && $pass_num == 1 && $pass_char == 1) {
        ...
        $encrypt_password = md5($password);     /* Password Encryption */
        ...
        if ($after_assoc['total_users'] == 0) {
            ...
            /* Success Message stored in SESSION layer and will show up in register page. */
            $_SESSION['success_msg'] = 'Congratulations! Registration successful.';
            header('location: register.php');
        } else {

            /* Error Message for existing user stored in SESSION layer and will show up in register page. */
